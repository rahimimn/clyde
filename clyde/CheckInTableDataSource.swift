//
//  CheckInTableDataSource.swift
//  clyde
//
//  Created by Rahimi, Meena Nichole (Student) on 8/21/19.
...
/// Protocol adopted by classes that wish to be informed when an `CheckInTableDataSource`
/// instance is updated.
protocol CheckInTableDataSourceDelegate: AnyObject {
...
class CheckInTableDataSource: NSObject {
    ...
    weak var delegate: CheckInTableDataSourceDelegate?
    ...
    @objc func fetchData() {
        ...
        RestClient.shared.send(request: request, onFailure: handleError) { [weak self] response, _ in
            ...
            DispatchQueue.main.async {
                ...
                self.delegate?.checkInTableDataSourceDidUpdateRecords(self)
            }
        }
    }
}

extension CheckInTableDataSource: UITableViewDataSource {
